...
func (e *Engine) Run() error {
	...
	dialSem := make(chan struct{}, resolvedDialConcurrency(e.DialConcurrency, e.Concurrency))
	...
	for i := 0; i < e.Concurrency; i++ {
		...
		go func(workerID int) {
			...
			for ip := range jobs {
				...
				openPorts := scanPorts(ctx, ip.String(), e.Ports, e.Timeout, resolvedPortConcurrency(e.PortConcurrency, len(e.Ports)), dialSem)
				if len(openPorts) == 0 {
					if e.Reporter != nil {
						e.Reporter.OnHostDone()
					}
...
func scanPorts(ctx context.Context, ip string, ports []int, timeout time.Duration, portConcurrency int, dialSem chan struct{}) []int {
	if len(ports) == 0 {
		return nil
	}
...
func resolvedDialConcurrency(requested int, hostConcurrency int) int {
	if requested > 0 {
		return requested
	}
	v := hostConcurrency * 32
	const max = 8192
	if v > max {
		return max
	}
